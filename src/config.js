...
if (!process.env.PRIVATE_KEY || !process.env.BASE_SEPOLIA_RPC_URL || !process.env.KITE_TESTNET_RPC_URL) {
    console.error("Error: Please ensure PRIVATE_KEY, BASE_SEPOLIA_RPC_URL, and KITE_TESTNET_RPC_URL are set in your .env file.");
    process.exit(1);
}
...
module.exports = {
    ...
    myAddress: new ethers.Wallet(process.env.PRIVATE_KEY).address,
    ...
    // Transaction Amount Ranges (min and max are inclusive)
    AMOUNT_ETH_RANGE: { min: 0.01, max: 0.012 },
    AMOUNT_KITE_TOKEN_RANGE: { min: 1.0, max: 1.2 },
    AMOUNT_USDT_RANGE: { min: 0.1, max: 0.15 },
    USDT_DECIMALS: 6, // Standard for USDT, adjust if different on this testnet

    // Time Delay Ranges in Minutes
